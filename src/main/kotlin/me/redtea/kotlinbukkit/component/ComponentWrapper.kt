package me.redtea.carcadecomponents

import me.redtea.serverbanksystem.messages.Container
...
class ComponentWrapper(var value: MutableList<String>, val container: Container) {
    ...
    fun toComponent(): TextComponent = (prefix?.let { mm.deserialize(it) } as TextComponent).toBuilder()
        .also { component ->
            value.map {
                mm.deserialize(it)
            }.mapIndexed { index, it ->
    ...
    fun replace(from: String, to: String): ComponentWrapper = ComponentWrapper(this.value.map { it.replace(from, to) }.toMutableList(), container)
